    ...
    <section>
      <div class="container">
        <div class="row">
          ...
          <div class="col-md-12 col-12" data-aos="fade-up">
            ...
            $sql_jum = "SELECT `p`.`id_post`, `p`.`judul`, `k`.`nama_kategori`, `u`.`nama`, `p`.`isi`, `p`.`image`, `p`.`date`, `p`.`status`, `p`.`trending`,`u`.`foto`,`p`.`headline`
                    ...
                    WHERE `p`.`trending` = 'Yes' and `p`.`status` = 'Publish'";
            $sql_jum .= " order by `p`.`date` desc ";
        ...
        $sql = "SELECT `p`.`id_post`, `p`.`judul`, `k`.`nama_kategori`, `u`.`nama`, `p`.`isi`, `p`.`image`, `p`.`date`, `p`.`status`, `p`.`trending`,`u`.`foto`,`p`.`headline`
                    ...
                    WHERE `p`.`trending` = 'Yes' and `p`.`status` = 'Publish' order by `p`.`date` desc limit $posisi, $batas";
                //echo $sql;
              ...
              if($jum_halaman==0){
                ...
                echo "<a class='page-link active'>1</a>";
                  ...
                  if($halaman!=1){
                    ...
                    href='index.php?include=hotnews&halaman=1'>First</a>";
                    echo "<a class='page-link prev'
                    href='index.php?include=hotnews&halaman=$sebelum'>«</a>";
                  }
                  for($i=1; $i<=$jum_halaman; $i++){
                    if ($i > $halaman - 5 and $i < $halaman + 5 ) {
                      if($i!=$halaman){
                        echo "<a class='page-link' 
                        href='index.php?include=hotnews&halaman=$i'>$i</a>";
                      }else{
                  ...
                  if($halaman!=$jum_halaman){
                    ...
                    href='index.php?include=hotnews&halaman=$setelah'>»</a>";
                    echo "<a class='page-link next' 
                    href='index.php?include=hotnews&halaman=$jum_halaman'>Last</a>";
                  }
